...
MOCK_CRTSH_RAW_DATA = [
    ...
    },
    {
        "issuer_ca_id": 1426,
        "issuer_name": (
            "C=US, O=DigiCert Inc, " "CN=DigiCert SHA2 High Assurance Server CA"
        ),
...
@pytest.mark.asyncio
@patch("asyncio.to_thread")
async def test_query_dns_generic_exception(mock_to_thread, caplog):
    """Test _query_dns handling generic exceptions."""
    mock_to_thread.side_effect = Exception("Generic DNS error")

    result = await _query_dns("exception.com", "A")

    assert result == []
    mock_to_thread.assert_awaited_once()
    # Check the specific log message generated by the exception handler
    assert "Error querying DNS A for exception.com: Generic DNS error" in caplog.text
    assert caplog.records[-1].levelname == "ERROR"
...
async def test_email_security_check_nxdomain(mock_query_dns, caplog):
    ...
    # (email_security_check/_check_spf/_check_dmarc)
...
async def test_email_security_check_noanswer(mock_query_dns, caplog):
    ...
    assert "Error checking SPF for example.com" in caplog.text
    assert (
        "No DMARC record found for example.com" in caplog.text
    )  # This is logged when DMARC lookup is empty
...
async def test_email_security_check_dmarc_policies(
    mock_query_dns, dmarc_record_content, expected_policy
):
...
async def test_virustotal_url_analysis_not_found(mock_client_cls, mock_getenv):
    ...
    mock_client_instance.get.return_value = (
        mock_response  # get() returns the 404 response
    )
...
async def test_crt_sh_lookup_success(mock_client_cls):
    ...
    # Check second certificate parsing and cleaning
    # (handles None common_name, extra newline)
    assert (
        result[1]["issuer_name"]
        == "C=US, O=DigiCert Inc, CN=DigiCert SHA2 High Assurance Server CA"
    )
...
async def test_ipwhois_lookup_success(mock_ipwhois_cls, mock_to_thread, mock_get_loop):
    ...
    mock_loop.getaddrinfo.assert_awaited_once_with(
        MOCK_DOMAIN, None, family=socket.AF_INET
    )
...
async def test_ipwhois_lookup_whois_exceptions(
    ...
    assert result is None
    if isinstance(
        ipwhois_exception, (ASNRegistryError, IPDefinedError, WhoisRateLimitError)
    ):
...
async def test_email_security_check_generic_exception(mock_query_dns, caplog):
    """Test email_security_check handling generic exceptions from _query_dns."""
    mock_query_dns.side_effect = Exception("DNS query failed unexpectedly")

    result = await email_security_check(MOCK_DOMAIN)

    # Should still return a dict, but with None values where lookups failed
    assert isinstance(result, dict)
    assert result.get("spf_record") is None
    assert result.get("dmarc_record") is None
    assert result.get("dmarc_policy") is None
    # Check logs for both SPF and DMARC attempts
    assert "Error checking SPF for example.com" in caplog.text
    assert "Error checking DMARC for example.com" in caplog.text
    # Ensure _query_dns was called for both checks
    assert (
        mock_query_dns.call_count >= 1
    )  # Should be 2 ideally, but depends on error point
...
# --- Tests for resolve_domain_tool (if exists) ---
# Assuming a resolve_domain_tool exists and uses socket.gethostbyname_ex


@pytest.mark.asyncio
@patch("tools.domain_tools.asyncio.get_running_loop")
async def test_resolve_domain_tool_gaierror(mock_get_loop, caplog):
    """Test resolve_domain_tool handling socket.gaierror."""
    mock_loop = AsyncMock()
    mock_get_loop.return_value = mock_loop
    mock_loop.getaddrinfo.side_effect = socket.gaierror("Test gaierror")

    # Assuming the tool exists and is imported
    from tools.domain_tools import ipwhois_lookup  # Using ipwhois as it resolves

    result = await ipwhois_lookup("gai-error-domain.com")

    assert result is None
    assert "Could not resolve domain gai-error-domain.com" in caplog.text
    mock_loop.getaddrinfo.assert_awaited_once_with(
        "gai-error-domain.com", None, family=socket.AF_INET
    )


@pytest.mark.asyncio
@patch("tools.domain_tools.asyncio.get_running_loop")
async def test_resolve_domain_tool_exception(mock_get_loop, caplog):
    """Test resolve_domain_tool handling generic Exception during resolution."""
    mock_loop = AsyncMock()
    mock_get_loop.return_value = mock_loop
    mock_loop.getaddrinfo.side_effect = Exception("Generic resolution error")
    ...
    result = await ipwhois_lookup("generic-error-domain.com")

    assert result is None
    assert (
        "Unexpected error during domain resolution for generic-error-domain.com"
        in caplog.text
    )
    mock_loop.getaddrinfo.assert_awaited_once_with(
        "generic-error-domain.com", None, family=socket.AF_INET
    )
